package com.quick.shelf.modular.creditPort.quickMoney.bill99.util;
...
public class ParseXMLUtil {
     ...
     */
    static ParseXMLUtil initParseXMLUtil() {
    ...
    private HashMap parseXMLNodeList(HashMap hm, Element e, String flag2) {
        Element child;   //定义一个Element元素对象
        Element child_txn;        //定义一个Element元素对象
        HashMap HM_txn;
        List list;
        ...
        for (Object o : e.getChildren()) {
            ...
            if (flag2.equals(child.getName())) {
                ...
                for (Object value : child.getChildren()) {
                    ...
                    child_txn = (Element) value; //获取每一个子元素
                    parseXMLNode(HM_txn, child_txn);//调用parseXMLNode函数，得到该节点下的所有元素
     ...
     */
    HashMap returnXMLData(Element root, String flag1, String flag2) {
        ...
        if (root != null) {
            ...
            for (Object o : root.getChildren()) {
                child = (Element) o; //获取每一个子元素
                childName = child.getName();  //得到该节点的名称

                //下面第一个判断，用来匹配第一个XML标志节点（如：TxnMsgContent）；
                //第二个判断，用来匹配第二个XML标志节点（如：ErrorMsgContent）
                if (flag1.equals(childName)) {
                    parseXMLNode(xmlData, child);//调用parseXMLNode函数，得到该节点下的所有元素
     ...
     */
    HashMap returnXMLDataList(Element root, String flag1, String flag2) {
        HashMap xmlData = null;    //定义一个HashMap对象
        Element child;            //定义一个Element元素对象
        ...
        if (root != null) {
            ...
            for (Object o : root.getChildren()) {
                ...
                if (flag1.equals(childName)) {
                    parseXMLNodeList(xmlData, child, flag2);//调用parseXMLNode函数，得到该节点下的所有元素
                } else if ("ErrorMsgContent".equals(childName)) {
                    parseXMLNode(xmlData, child);//调用parseXMLNode函数，得到该节点下的所有元素
                } else {
                    xmlData.put(childName, child.getValue());  //将子元素保存到HashMap中
